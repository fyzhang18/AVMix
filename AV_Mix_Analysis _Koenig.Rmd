...
doo <- ddply(poo5,.(subID,block,trialtype),summarise,avg.pupilSs=mean(PUPIL_CORRECTED,na.rm = TRUE)) 
doo2 <- ddply(doo,.(block,trialtype),summarise,avg.pupil=mean(avg.pupilSs,na.rm = TRUE),sePupil=sd(avg.pupilSs, na.rm = TRUE)/sqrt(length(avg.pupilSs)))
...
foo <- ddply(poo5,.(subID,block,trialtype),summarise,avg.pupilSs=mean(PUPIL_CORRECTED,na.rm = TRUE)) 
foo2<-foo[!(foo$trialtype==3 | foo$trialtype==4 | foo$trialtype==6),] 

fit <- aov(avg.pupilSs~(trialtype*block)+Error(subID/(trialtype*block)),
  	data=foo2)
summary(fit)

#main effect of trial type p = 0.0579
#no effect of block p = 0.4765
#no interaction p = 0.8447

######### Correlation #########
foo3 <- ddply(foo2,.(block,trialtype),summarise,avg.pupil=mean(avg.pupilSs,na.rm = TRUE),sePupil=sd(avg.pupilSs, na.rm = TRUE)/sqrt(length(avg.pupilSs)))
...
doo <- ddply(poo6,.(subID,TIMECODE,block,trialtype),summarise,avg.pupilSs=mean(PUPIL_CORRECTED,na.rm = TRUE)) 
doo2 <- ddply(doo,.(TIMECODE,block,trialtype),summarise,avg.pupil=mean(avg.pupilSs,na.rm = TRUE),sePupil=sd(avg.pupilSs, na.rm = TRUE)/sqrt(length(avg.pupilSs)))
...
ggplot(doo3,aes(x=TIMECODE,y=avg.pupil,fill=factor(label),colour=factor(label)))+
  geom_line()+
  ggtitle("Pupil change")+
  theme_bw()+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line = element_line(colour = "black"))+
  ...
  theme(strip.text = element_text(size=16))+
  theme(plot.title = element_text(hjust = 0.5))+
  scale_color_manual(values=c("#2a92ff", "#ff15ff", "#559455"),name="Trial type",breaks=c("1", "2","3"),labels=c("Low uncertainty, low reward", "High uncertainty, moderate reward","Low uncertainty, high reward"))+
